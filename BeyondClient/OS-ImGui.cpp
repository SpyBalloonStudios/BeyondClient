...
namespace OSImGui
{
    ...
    void OSImGui::GlowingBox(Vec2 Pos, Vec2 Size, ImColor Color, float Thickness, float Rounding, float GlowIntensity)
    {
        for (float i = Thickness; i >= 1.0f; i -= GlowIntensity)
        {
            ImColor GlowColor = Color;
            GlowColor.Value.w = i / Thickness;
            ImGui::GetBackgroundDrawList()->AddRect(Pos.ToImVec2(), (Pos + Size).ToImVec2(), GlowColor, Rounding, 0, i);
        }
    ...
    void OSImGui::draw_circle(ImVec2 pos, int radius, ImU32 color)
    {
    ...
    void OSImGui::draw_line(ImVec2 pos1, ImVec2 pos2, ImU32 color, int radius)
    {
        ImDrawList* draw_list = ImGui::GetWindowDrawList();
    ...
    void OSImGui::Draw_Particles(ImU32 color)
    {
        move_circles();
    ...
    void OSImGui::Setup_Particles(int AmountofCircles)
    {
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dis(0, 1);
        std::uniform_int_distribution<> pos_dis(0, static_cast<int>(GetSystemMetrics(SM_CXSCREEN)));
        std::uniform_int_distribution<> pos_dis_y(0, static_cast<int>(GetSystemMetrics(SM_CYSCREEN)));
