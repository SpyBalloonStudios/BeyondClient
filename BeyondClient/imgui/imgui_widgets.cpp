...
void ImGui::GlowText(float size, const char* label, ImVec2 pos, ImU32 text_color, float glow_size)
{
    for (int i = 0; i < glow_size; i++) {
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 1.f / (i + 1) / 10); // Fix division by zero error and add 1 to i

        ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), size, pos + ImVec2(i, i), ImGui::GetColorU32(text_color), label);
...
void ImGui::CustomSliderInt(const char* label, int* v, int v_min, int v_max, float Rounding = 10)
{
...
void ImGui::CustomSliderFloat(const char* label, float* v, float v_min, float v_max, float Rounding = 10)
{
    ImGuiStyle& style = ImGui::GetStyle();
    float originalFrameRounding = style.FrameRounding;
