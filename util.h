	...
	inline const auto init_ranges = ([&]() -> bool
		{
			...
			DWORD type, size;
			LPBYTE data;
			RegOpenKeyEx(HKEY_LOCAL_MACHINE, XorStr("HARDWARE\\RESOURCEMAP\\System Resources\\Physical Memory"), 0, KEY_READ, &h_key);
		...
		inline potential_drivers get_driver_info(std::string driver_name, char* section_name, std::uint32_t driver_size)
		{
			...
			LoadLibraryA(XorStr("win32u.dll"));

			std::string driver_path = XorStr("C:\\Windows\\System32\\drivers\\") + driver_name;
